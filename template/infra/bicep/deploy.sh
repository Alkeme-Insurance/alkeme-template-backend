#!/bin/bash
# ============================================
# {{ project_name }} - Bicep Deployment Script
# ============================================
# Deploys ACR, AKS, {% if deploy_cosmos_db %}Cosmos DB, {% endif %}and supporting resources
...
GITHUB_REPOSITORY="{% if repository_url %}{{ repository_url.split('/')[-2:] | join('/') }}{% endif %}"  # Format: owner/repo
...
log_info "Starting {{ project_name }} infrastructure deployment..."
echo ""
...
{% if use_azure_auth %}
            ...
            AZURE_TENANT_ID|VITE_AZURE_TENANT_ID)
                ...
                ;;
            AZURE_API_SCOPE|VITE_AZURE_API_SCOPE)
...
{% if use_azure_auth %}
    ...
    if [ -z "$AZURE_CLIENT_ID" ] || [ "$AZURE_CLIENT_ID" == "" ]; then
        ...
        log_warning "This is fine for dev mode (DEV_NO_AUTH=true)"
    else
...
{% if use_azure_auth %}
    ...
    AZURE_API_SCOPE=""
{% endif %}
    BACKEND_CORS="http://localhost:8000"
fi
echo ""
...
echo "   ${GREEN}docker build -t $ACR_LOGIN_SERVER/{{ package_name }}-backend:latest -f Dockerfile .${NC}"
echo "   ${GREEN}docker push $ACR_LOGIN_SERVER/{{ package_name }}-backend:latest${NC}"
echo ""
echo "5. Create Kubernetes secrets from Key Vault:"
echo "   ${GREEN}cd ../k8s${NC}"
echo "   ${GREEN}export KEY_VAULT_NAME=\"$KEY_VAULT_NAME\"${NC}"
echo "   ${GREEN}kubectl create namespace {{ package_name }}${NC}"
echo "   ${GREEN}kubectl create secret generic {{ package_name }}-secrets \\${NC}"
echo "   ${GREEN}  --from-literal=mongodb-uri=\"\$(az keyvault secret show --vault-name \$KEY_VAULT_NAME --name mongodb-uri --query value -o tsv)\" \\${NC}"
{% if use_azure_auth %}
echo "   ${GREEN}  --from-literal=azure-client-id=\"\$(az keyvault secret show --vault-name \$KEY_VAULT_NAME --name azure-client-id --query value -o tsv)\" \\${NC}"
echo "   ${GREEN}  --from-literal=azure-tenant-id=\"\$(az keyvault secret show --vault-name \$KEY_VAULT_NAME --name azure-tenant-id --query value -o tsv)\" \\${NC}"
{% endif %}
echo "   ${GREEN}  --namespace {{ package_name }}${NC}"
echo ""
echo "6. Update Kubernetes manifests with ACR login server:"
echo "   ${GREEN}# Edit backend-deployment.yaml and replace <ACR_LOGIN_SERVER> with:${NC}"
echo "   ${GREEN}# $ACR_LOGIN_SERVER${NC}"
echo ""
echo "7. Deploy to Kubernetes:"
echo "   ${GREEN}kubectl apply -f ./${NC}"
echo ""
echo "8. Check deployment status:"
echo "   ${GREEN}kubectl get pods -n {{ package_name }}${NC}"
echo "   ${GREEN}kubectl logs -f deployment/backend -n {{ package_name }}${NC}"
echo ""
...
if [ -n "$GITHUB_REPOSITORY" ]; then
    ...
    echo "9. Configure GitHub Actions:"
...
cat > "$OUTPUT_FILE" << EOF
{{ project_name }} Deployment Outputs
==============================
...
- mongodb-uri {% if deploy_cosmos_db %}(Cosmos DB connection string){% endif %}
{% if use_azure_auth %}
...
5. Create Kubernetes secrets from Key Vault
